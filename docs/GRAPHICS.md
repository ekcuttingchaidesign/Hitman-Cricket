...
**A. Keep building them in code.** Nobody models anything: every shape is
generated by three.js at start-up, as the players are today. Part 3 steps 1–5,
with the big head kept, give smooth bending limbs, a shaped body and a sculpted
helmet with its grille and neck guard. Painted textures, also drawn in code,
add the shirt number, the bat sticker and the stripes on the boots. The target
batter is seen from behind with his face inside the helmet, so this gets closer
than it might seem: the silhouette, the colours and the kit all match. What code
cannot easily do is the fine surface detail: shirt folds, and the soft sculpted
look of cloth and skin. Up close, route A reads as a clean, smooth toy figure
rather than the image's hand-sculpted one.

**B. A modelled stylised batter.** A 3D model in the image's style and
proportions (big helmet, stocky body, jersey number), exported as a skinned
glTF and driven by the game's existing poses (item 17), so every stroke that
exists today is kept exactly. It doesn't have to be modelled by the game's
owner. It can be:

- commissioned from a 3D artist, who can work from the target image;
- bought or downloaded (stylised sports characters exist on the asset stores,
  and some free ones are CC0);
- generated from the target image with an image-to-3D tool, then cleaned up
  and rigged.

Whichever source it comes from, the code side (loading it, attaching the bat,
driving its bones from the existing poses) is ordinary three.js work.

**Recommended: start with route A for everyone.** It needs no art and no new
download, and all of it carries over if route B follows. Judge the batter once
phase 1's lighting is in, since good lighting is much of what makes the target
image look finished. Only if his surface still falls short, bring in a modelled
batter (route B) for him alone. The fielders are 40 pixels tall, and route A is
more than enough there either way.
...
3. **The players (3–6 weeks).** Route A for everyone, then route B for the
   batter only if he still falls short. Most of the uncertainty in the whole
   plan is here: whether a modelled batter is wanted, where it comes from, and
   how much tuning the strokes need on a new body.
