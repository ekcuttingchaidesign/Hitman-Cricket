...
function startReview() {
  ...
  function filename(extension: string) { return `hitman-pass04-${state.action}-${state.time}ms-${state.kit}-${state.frame}.${extension}`; }
  ...
  el('save-settings').onclick = () => {
    ...
    download(new Blob([JSON.stringify({ baselineCommit: 'd630eb4', graphicsPass: '04', state, viewport: FRAMES[state.frame],
