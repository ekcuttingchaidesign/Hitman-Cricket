...
const ballFor = (s: string) => s === 'PULL' ? { shot: 'LEG' as const, x: -.30, y: 1.12 }
  ...
  : { shot: 'STRAIGHT' as const, x: 0, y: .50 };
...
for (let i = 0; i < frames; i++) {
  ...
  batter.swing(spec.shot, 0, spec.x, spec.y, GAME.contactZ, shot === 'CHARGE');
  batter.update(age);
