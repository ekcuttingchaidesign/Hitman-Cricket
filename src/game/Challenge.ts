...
import {
  answerChallenge, challengeLink, challengeText, challengeWhatsapp, clearUnsent, codeFromLocation,
  fetchChallenge, forgetChallenge, holdUnsent, openChallenges, readUnsent, rememberChallenge,
  resultText, setChallenge, type Challenge, type ChallengeRow,
} from './challenge-api';
...
export class ChallengeRun {
  ...
  static async answered(playerId: string): Promise<Challenge[]> {
    const codes = openChallenges();
    if (!codes.length) return [];
    const reads = await Promise.all(codes.map(code => fetchChallenge(code)));
    const done: Challenge[] = [];
    reads.forEach((answer, i) => {
      // A code that has closed is dropped; one that simply could not be reached
      // is left alone, because a dead spot is not an answer.
      if (!answer.ok && !answer.retry) forgetChallenge(codes[i]);
      if (answer.ok && answer.challenge?.state === 'answered') {
        forgetChallenge(codes[i]);
...
export function resultView(challenge: Challenge, you: string, link: string): ChallengeResultView {
  ...
  const tag = allOut(mine) ? `ALL OUT · BALL ${mine?.balls ?? 0}` : `${them.toUpperCase()}'S CHALLENGE`;
  const title = won ? 'You win' : margin === 0 ? `${them} wins` : `${them} wins`;

  let sub: string;
  if (won && allOut(mine)) {
    sub = `Three down inside ${overs(mine)} and still past ${them}'s ${theirRuns}. Reckless. Effective.`;
  } else if (won) {
    sub = `By ${margin} run${margin === 1 ? '' : 's'}. ${them} is not going to enjoy this.`;
  } else if (margin === 0) {
    ...
    sub = `${margin} run${margin === 1 ? '' : 's'} short, three down inside ${overs(mine)}. Brutal, honestly.`;
  } else {
    sub = `${margin} run${margin === 1 ? '' : 's'} short. So close it hurts.`;
  }
