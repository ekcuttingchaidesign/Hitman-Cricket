  ...
  constructor(root: HTMLElement, best: number) {
    ...
    root.innerHTML = `
      <header class="topbar">
        ...
        <div class="header-actions"><button id="sound" class="icon-button" aria-label="Mute sound" title="Sound (M)">${icon('sound')}</button><button id="help" class="icon-button" aria-label="How to play" title="How to play">${icon('help')}</button><button id="share" class="icon-button" aria-label="Share game" title="Share game">${icon('share')}</button><span class="desktop-badge">SWIPE OR KEYS</span></div>
      </header>
      ...
      <main>
        ...
        <div class="game-layout">
          <section class="arena" aria-label="Cricket batting game">
            ...
            <div class="swipe-guide touch-only" aria-label="Swipe directions"><span>← Leg</span><span>↖ Long on</span><span>↑ Straight</span><span>↗ Cover</span><span>→ Off</span></div>
          </section>
          <aside class="guide"><div class="guide-title"><span class="eyebrow">THE PLAYBOOK</span><span class="guide-number">01—05</span></div><h2>Pick your shot.</h2><p>Watch the line. Swing as the ball<br class="wide-only"> reaches your bat.</p>
            <div class="shot-list">${controls.map(([shot, keys, label, arrow]) => `<div class="shot-row" data-shot="${shot}"><span class="shot-direction">${arrow}</span><span class="shot-name">${label}</span><span class="key-group keyboard-only">${keys.split(' + ').map(k => `<kbd>${k}</kbd>`).join('<span>+</span>')}</span></div>`).join('')}</div>
        ...
        <footer class="keyboard-only"><span><span class="small-dot"></span> NO DOWNLOADS. JUST CRICKET.</span><span><kbd>Esc</kbd> Pause <i></i> <kbd>M</kbd> Sound <i></i> <kbd>R</kbd> Restart</span><span>BUILT FOR THE LOVE OF THE GAME.</span></footer>
      </main>
