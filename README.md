...
What is ranked is the **bar, not the blow count** beside it: one on the helmet costs thirty and two on the pad sixteen, so a ladder reading the tally would have said the opposite of what the player watched. It is normalized into twenty-five steps across the full bar, because twenty-five steps need five bits while the raw meter would need seven. Nothing is lost by the rounding — the cheapest blow in the mode still costs several points, so any two innings that took a different battering are at least one step apart. Normalizing also keeps leaderboard scores comparable when the meter's tuning changes: a completely fresh batter is step twenty-five either way.
