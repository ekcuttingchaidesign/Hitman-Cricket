...
The start screen offers a four-ball tutorial. Each ball is slow, dead straight, and scripted to teach one gesture: a middle-stump ball to drive straight (swipe up), one on the legs to whip away square (swipe right to left), one wide outside off to punch through the covers (swipe left to right), and one wide again to cut square off the back foot (swipe down towards the off side). An arrow on the field sweeps the way you must swipe. Nobody gets out, nothing counts towards a score, and **Skip to innings** leaves at any point.

## The square cut

A sixth scoring stroke, played off the back foot and square of the wicket: **swipe down towards the off side**, or press <kbd>D</kbd> and then <kbd>S</kbd>. He rocks back and across, frees his arms at the ball, strikes it square with a horizontal bat, and the arc keeps going — the wrists roll, the body unwinds on the back foot, and the blade wraps up over the front shoulder. That follow-through is what separates it from the off-side punch on <kbd>D</kbd>, which stays a punch and is unchanged.

It wants width. Compatibility runs 1 outside off, 0.7 on off, 0.2 on middle and nothing on the leg side, and `CUT.minWidth` is how far outside off the ball has to finish before there is room to play it at all — read off where the ball ends up, so a ball swinging away into the cut is cuttable and one coming back in is not. The bat's own reach starts at that same width, so a cut swung at a straight ball plays square of the stumps and the ball goes past it.

It is also the one answer to a bouncer outside off besides the pull. A short ball is judged on timing alone: middled it is six, well timed four, held back it is worked along the ground, and anything later or earlier feathers the edge to the keeper. Off a length with width it scores the same way through the ordinary path. An edge is not a skier — it flies off the face at gloves height and is taken behind the stumps, so it is placed where the keeper stands rather than swept out along the stroke's angle.

## Mobile controls

Swipe **left**, **up-left**, **up**, **up-right**, or **right** for leg side, long on, straight, cover, or off side, **down-right** for the square cut, and **down** to block. Every stroke owns a 45-degree sector. The cut takes its sector out of the block's off-side half, which is the one direction a stroke now competes for: down and to the leg the block keeps all the room it had, and the sliver beyond that stays dead, so a sideways drag is still no shot. A short 24-pixel swipe commits the shot immediately when its direction becomes clear. Timing is measured at that moment, not at finger-down or release, and uses the same timing bands, compatibility, scoring, and wickets as keyboard play. One shot per ball is shared across input methods. Taps, second fingers, and cancelled gestures do not trigger a shot. A gesture cannot carry into the next ball.

On a keyboard the cut is <kbd>D</kbd> and then <kbd>S</kbd>, in that order: it is the fourth diagonal alongside <kbd>A</kbd>+<kbd>W</kbd> and <kbd>W</kbd>+<kbd>D</kbd>, but <kbd>S</kbd> pressed first still commits to the block at once rather than waiting out the combo window, because reaching for the block first is deciding not to play a stroke.
