...
Twenty-five bits of rank over twenty-eight of clock, which is a double's fifty-three exactly. The meter's five bits took the last of the room this ladder had spare, so it now sits where the other one does: at the line, with nothing left. A test holds it there, because a field added after this cannot be paid for out of the rank — it would have to come out of the clock, and shortening the clock reshuffles ties that are already settled. The clock stays at the bottom regardless: it is the only key the store stamps itself, so it is the one figure a submitted row cannot lie about. The tier is read off the figures rather than carried beside them, because the store has to decide it from a row somebody posted and cannot trust an ending sent along with it. The order matches the game's own `endingOf`: the hundred is checked first, so a chase finished on the last ball is a win; the wicket next, because out is out whenever it happened — a man bowled on the sixtieth ball is all out and has lost, and he tops the losing tier rather than joining the draws, since no loser kept them out longer. Only the meter is read after the overs: a blow that empties it as the tenth over finishes cut nothing short, so it is still a draw.

A row is a table row and nothing more: **place · letter · kit · name · runs · balls · blows**, with the three figures at one size, one weight and in the same place every time. It went through a louder draft first — the ranked figure blown up on each row — and that draft was wrong in a way worth recording: there is no one hero number on this board, so blowing up the ranked figure blew up a *different* figure from tier to tier, and the moving emphasis read as the ranking itself. Three quiet columns say it in one place, and the units are drawn once at the head of the list rather than fifty times down it.

**The letter is the whole of what a row says about how the innings ended** — **W**, **D**, **L**, **R** — in the sheet's tier colours, where a league table puts it. Four rather than three, because a loss comes two ways: bowled out, or carried off. That difference is the whole of what the injury bar is for and it used to cost a line of prose under every name; a fourth letter says it in a column that was already there. The colour never carries it alone: the letter says it, and the word behind the letter (`Bowled out`, `Retired hurt`) is what a screen reader hears, which is the only voice a row has now that the prose is gone.

**What orders a block is said once, above it**: *Wins · fewest balls used*, *Draws · most runs*, *Losses · longest innings*. The rule belongs to a whole block rather than to a row — fifty copies of it was the clutter — and it cannot live only in the footer, which is below the scroll and answers a question the reader is asking here. The tiers are contiguous, so three bands cover fifty rows.

That leaves one rung with nowhere to write itself. The meter has no column, so where the battering is what put a row under the one above it, **the blows are lit** instead of explained — the same thing the five-over sheet does with the figure that settled a tie. Blows are not the meter, since a helmet costs more than a pad, but they are its shadow, and a reader who looks at the lit figure is looking at the right thing. A pair level even on the meter was split by the clock, which no figure on the row can show; the footer says it.
...
**The sheet carries both ladders as tabs** — *The Blast* and *Test Survival*, in the dark above it, which was the one piece of that screen carrying nothing. Before them the board you could reach was the board for the innings you were in, and the other one was only reachable by playing the other mode. The sheet always **opens on the mode the player came from**: the tab is a way *out* of the board they asked for, not a question about which one they meant. Tabs are drawn only where both ladders exist — a build that plays one mode has one board, and a row of tabs over it would be two names for one thing.

**The tabs do not scroll away.** The fifty rows scroll inside the sheet rather than moving it, so the tabs and the sheet's own head stay where they are; and on a screen too short to hold the sheet's fixed parts, where the overlay has to scroll for the foot of it to be reachable at all, the tabs are sticky. Making that true meant containing the list's paint: without `contain:paint` on the scroller its rows still counted towards the *overlay's* scrollable height, so fifty of them made the backdrop scrollable by fourteen hundred pixels, and a flick anywhere off the list dragged the whole sheet off the top of the screen and left the ground behind it.

Three things follow from the sheet being shared. The innings just played peeks on its own board and on no other, so switching from a Test card shows the Blast's fifty and nothing of yours. A fetch that lands after the player has tabbed away is dropped rather than drawn, or it would put the ladder they just left back under the tab they just chose. And the card's keys at the foot of the sheet — the ones a claim leaves behind — are held across a tab, because a player who has taken a place, looked at the other ladder and come back has not given up their way to play again.
...
- `src/entities/BatterModel.ts`: the modelled batter, worn over the code-built one. The solver below stays the authority on every joint and on where the blade is; each frame its solved hips, chest, head, elbows, knees, feet and bat are handed to this class, which poses a skinned character to match — hands glued to the bat the way the model's own rest pose grips it, arms and legs solved back from there with the model's own bone lengths, stretching a little at full reach rather than letting the gloves leave the handle. The asset is `public/models/batter.glb`, built from the authored Blender export by `scripts/batter-build.mjs` (one scene, the stitching and construction curves dropped, every mesh simplified with its skin weights kept, rigid parts baked onto their bones and merged by material: about 82k triangles and 2 MB from a million and 23 MB). It wears the kit it was authored in; only whites repaint it, by material name. Limbs that must reach further than the model's own lengthen along the bone only, and the hips drop rather than the legs stretching. `tools/model-lab.html` shows it through four moments of the drive and the cut from the game's camera and from square (`?set=b` for the pull and the scoop), which is where a bad bend shows first. If the file never arrives the code-built batter bats, so nothing waits on it.
