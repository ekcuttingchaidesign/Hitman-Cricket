...
Use the on-screen Pause button to resume, restart, or change mode. Change Mode opens the mode picker over the pause card, so backing out of the picker leaves the innings exactly where it was; picking a mode starts a fresh innings in it, and is counted as an innings walked out on the way a restart is. A link that names a mode (`?mode=`) hides the key, since there is nothing to change to. Page scrolling is suppressed on the field during play; dialogs can still scroll on small screens.
