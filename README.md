...
Its way back to the guard is authored too. A follow-through that wraps the bat behind the front shoulder cannot travel home in a straight line: the guard holds the bat behind the *other* shoulder, and every short path between the two goes through the batter's head and then his chest. The stroke names a `recover` pose — blade swung forward out in front of him, where he can see it, before it drops into the pick-up — and the tail of the animation routes through that. Measured against the helmet and the trunk across the whole delivery range, the cut is now the cleanest of the cross-bat strokes.
